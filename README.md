<div align="center">
  ...
  <p>Lightweight · Type-safe · Dark mode ready</p>
...
- **🎨 Clean Architecture** — Separation of concerns (context, components, theme, app).
...
│   └── AnimatedThemeContext.tsx # Reanimated transitions
│
├── theme/                  # Design tokens
│   └── colors.ts           # Color palettes & type definitions
│
...
Edit [`src/theme/colors.ts`](src/theme/colors.ts) to define your brand palette for both light and dark modes.

### Fonts

Replace the `.ttf` files in `assets/fonts/`, then update the font loading in [`src/app/_layout.tsx`](src/app/_layout.tsx).
